...
|FORMAT.BAS| |
|FULLNAME.BAS| EXE |
|GASGAUGE.BAS| INCLUDE |
|GET1STR.BAS| |
|GETATTR.BAS| |
|GETCMOS.BAS| |
|GETCOLOR.BAS| EXE |
|GETDIR.BAS| EXE (check) |
|GETEQUIP.BAS| EXE |
|GETVMODE.BAS| EXE |
|GRAFCURS.BAS| |
|HAND2NAM.BAS| |
...
|PARSESTR.BAS| EXE |
|PAUSE3.BAS| EXE |
|PICKLIST.BAS| INCLUDE |
|PRO.BAS| INCLUDE (NEW)|
|PRO.BI| INCLUDE (NEW)|
|PRTSC.BAS| |
|PULLDNMS.BAS| INCLUDE |
|PULLDOWN.BAS| INCLUDE |
|PUTVOL.BAS| |
|QD.BAS| |
|QEDIT.BAS| INCLUDE |
|QEDIT7.BAS| |
...
Files marked with *INCLUDE* are meant to be included, while files marked as *EXE* are to be compiled as an executable (stand-alone example). There are a few *new* files added to the list and are indicated as such. The replacement of the PRO.LIB functionality is included in the *PRO.BAS* and *PRO.BI* source files.
